    ...
    def create_product_list_window(self):
        ...
        with dpg.window(label="Product List", tag="list_window",
            ...
            with dpg.table(tag="product_table", header_row=True,
                ...
                dpg.add_table_column(label=" Price ", width=500)
                dpg.add_table_column(label=" UPC Code ", width=500)
                dpg.add_table_column(label=" Exp Date ", width=500)
        ...
        for i, product in enumerate(self.model.get_all_products()):
            with dpg.table_row(parent="product_table") as row:
                for col_idx, value in enumerate([
                    ...
                    #         dpg.add_button(
                    #             label="Edit",
                    #             callback=lambda row=row, col_idx=col_idx, value=value: self.create_edit_popup(row, col_idx, value)
                    #         )
                    #         dpg.add_button(
                    #             label="Delete",
                    #             callback=lambda row=row: self.delete_selected_row(row)
                    #         )
