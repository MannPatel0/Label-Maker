...
class LabelMakerApp:
    ...
    def setup_dpg(self):
        dpg.create_context()
        dpg.create_viewport(title="Label Maker v1.2.6", width=1130, height=627)
        dpg.setup_dearpygui()
    ...
    def create_menu(self):
        with dpg.viewport_menu_bar():
            ...
            #     dpg.add_menu_item(label="Create Labels", callback=self.create_help_window)
            #     # dpg.add_menu_item(label="Export Products", callback=self.export_products)
    
    def create_help_window(self):
        # Ensure the window can be closed with "X"
        with dpg.window(label="Help & Tutorial", modal=True, tag="help_window", width=600, height=400):
            
            dpg.add_text("Welcome to the Label Maker App!", wrap=550)
            dpg.add_separator()
            
            # Scrollable help text
            with dpg.child_window(height=300, width=-1, autosize_x=True, autosize_y=True):
                dpg.add_text(
                    """1. Click 'Open CSV' to select a file containing label data.\n
                        2. Adjust settings such as label size and font.\n
                        3. Click 'Generate Labels' to preview.\n
                        4. Click 'Save as PDF' to export labels.\n
                        5. For any issues, check the Help section or restart the app.\n
                        \n
                        Pro Tip: Use high-quality label paper for best results!
                    """,
                    wrap=550
                )
    ...
    def create_csv_import_window(self):
        ...
        with dpg.window(label="CSV Import", tag="csv_window",
            no_close=True, no_collapse=True, pos=[5,280],width=400, height=300, no_resize=False, no_move=True):
            ...
            dpg.add_text("Loading...", tag="csv_status")
    ...
    def callback(self, sender, app_data):
        ...
        print("1. File Path: ", app_data['file_path_name'])
        self.select_csv_file(app_data['file_path_name'])
    ...
    def select_csv_file(self, csv_file_path):
        ...
        try:
            ...
            dpg.set_value("csv_file_label", f"Selected: {csv_file_path}")
